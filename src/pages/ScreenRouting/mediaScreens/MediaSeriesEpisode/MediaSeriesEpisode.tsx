...
const MediaSeriesEpisode: ScreenComponent<PlaylistItem> = ({ data }) => {
  ...
  const filterMetadata = ` ${t('video:season')} ${episodeItem.seasonNumber}/${filters.length} - ${t('video:episode')} ${episodeItem.episodeNumber
    }/${episodesInSeason}`;
  ...
  return (
    <React.Fragment>
      ...
      <VideoLayout
        item={episodeItem}
        title={inlineLayout ? getTitle() : getSeriesTitle() }
